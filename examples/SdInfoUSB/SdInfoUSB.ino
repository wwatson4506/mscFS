...
void errorPrint() {
  if (msc1.sdErrorCode()) {
    ...
    printSdErrorSymbol(&Serial, msc1.sdErrorCode());
    cout << F(" = ") << int(msc1.sdErrorCode()) << endl;
    cout << F("msc1 errorData = ") << int(msc1.sdErrorData()) << endl;
  }
}
//------------------------------------------------------------------------------
bool mbrDmp() {
...
void loop() {
  ...
  if (!msc1.cardBegin(&msDrive1)) {
	...
	Serial.println(msc1.sdErrorCode());
    return;
  }
  t = millis() - t;
